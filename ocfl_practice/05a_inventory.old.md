...
Hier ist das `inventory.json` aus unserem Workshop-Beispiel für die Version `v1`:
...
## Erläuterung der Schlüsselbereiche

### 1. Header-Informationen
- **`id`**: Die eindeutige Identifikator des Objekts (`urn:nbn:de:gbv:42-test1`).
- **`type`**: Verweis auf die verwendete OCFL-Spezifikation (hier 1.1).
- **`digestAlgorithm`**: Der Haupt-Hash-Algorithmus (Standard: `sha512`). Er wird für das Manifest und die Zustandsbeschreibungen verwendet.
- **`head`**: Die aktuellste Version des Objekts (hier `v1`).
- **`contentDirectory`**: Der Name des Verzeichnisses innerhalb jeder Version, das die physischen Dateien enthält (Standard: `content`).

### 2. Manifest
Das `manifest` ist eine flache Liste aller physisch im Objekt vorhandenen Dateien. Jede Datei wird über ihren Hash-Wert referenziert. Der Hash ist der Schlüssel, und der Wert ist ein Array von Pfaden (da OCFL Deduplizierung unterstützt, könnte ein Hash auf mehrere Pfade verweisen).

### 3. Versions
Hier wird die Historie des Objekts abgebildet. Jede Version (z. B. `v1`) enthält:
- **`created`**: Zeitstempel der Erstellung.
- **`message`**: Beschreibung der Änderung.
- **`user`**: Wer die Änderung vorgenommen hat.
- **`state`**: Dies ist der **logische Zustand** der Version. Er bildet die ursprüngliche Dateistruktur auf die Hashes im Manifest ab. Hier sieht man die "sauberen" Namen (z. B. mit Leerzeichen), während das Manifest die physischen Pfade (mit URL-Encoding) enthält.

### 4. Fixity
Der `fixity`-Block ist optional und enthält alternative Hashes (wie `md5`, `sha1` oder `blake2b-384`) für die im Manifest gelisteten Dateien. Dies dient der langfristigen Integritätsprüfung mit verschiedenen Algorithmen.

---

[Zurück zur Objektstruktur](../05_object_structure) | [Zurück zum Inhaltsverzeichnis](../toc) | [Nächstes Thema: Objekt aktualisieren](../06_update_object)
